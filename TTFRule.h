...
struct TTFRule
{
	...
	TTFRule(std::string &name, int d, double w, double tru, unsigned long long tau, double trl, unsigned long long tal) :
		processName(name), direction(d), when(w),
		transferRateUpper(tru*1024), transferAmountUpper(tau*1024), transferRateLower(trl*1024), transferAmountLower(tal*1024)
	{}
	TTFRule(const char *name, int d, double w, double tru, unsigned long long tau, double trl, unsigned long long tal) :
		processName(std::string(name)), direction(d), when(w),
		transferRateUpper(tru*1024), transferAmountUpper(tau*1024), transferRateLower(trl*1024), transferAmountLower(tal*1024)
	{}
};
