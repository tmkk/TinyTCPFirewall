		...
		if(sections[pos] == 0) {
			...
			if(!strncasecmp(section,"rule",4)) {
				...
				if(exeName[0] != 0) {
					...
					//fprintf(stderr,"%s,%d,%d,%d,%d,%d,%d\n",exeName,direction,when,transferRateUpper,transferAmountUpper,transferRateLower,transferAmountLower);
					rules.push_back(PTTFRule(new TTFRule(exeName,direction,when,transferRateUpper,transferAmountUpper,transferRateLower,transferAmountLower)));
				}
			}
			else if(!strcasecmp(section,"global")) {
				blackListThreshold = GetPrivateProfileInt(section,"BlackListThreshold",0,iniPath);
				char addrStr[32];
				GetPrivateProfileString(section,"MyIPAddress","",addrStr,32,iniPath);
